'use client';
import { useState, useEffect } from 'react';

// ── Tipi ──────────────────────────────────────────────────────────────────────
interface Buco {
  roomId:    number;
  roomName:  string;
  property:  string;
  liberaDal: string;
  liberaAl:  string;
  notti:     number;
}

interface PuliziaRoom {
  roomId:   number;
  roomName: string;
  property: string;
  mq:       number;
}

interface PuliziaArrivo {
  roomId:   number;
  roomName: string;
  property: string;
}

interface PuliziaDay {
  date:       string;
  departures: PuliziaRoom[];
  arrivals:   PuliziaArrivo[];
  mqTotale:   number;
  rischio:    'CRITICO' | 'ALTO' | 'MEDIO' | 'BASSO';
}

interface OperativoData {
  buchi:              Buco[];
  pulizie:            PuliziaDay[];
  totalePrenotazioni: number;
  lastUpdated:        string;
}

// ── Stili base (stessa palette di AdminCheckin) ───────────────────────────────
const card: React.CSSProperties = {
  background: '#fff', borderRadius: 12,
  border: '0.5px solid #e5e7eb', padding: '16px 18px', marginBottom: 10,
};
const btnSec: React.CSSProperties = {
  padding: '8px 14px', fontSize: 13,
  background: 'none', color: '#6b7280',
  border: '0.5px solid #d1d5db', borderRadius: 8, cursor: 'pointer',
};
const btnG: React.CSSProperties = {
  padding: '10px 20px', fontSize: 14, fontWeight: 700,
  background: '#16a34a', color: '#fff', border: 'none',
  borderRadius: 8, cursor: 'pointer',
};

// ── Colori rischio ────────────────────────────────────────────────────────────
const RISCHIO_CONFIG = {
  CRITICO: { bg: '#FEE2E2', color: '#7f1d1d', border: '#fca5a5', emoji: '🔴' },
  ALTO:    { bg: '#FEF3C7', color: '#78350f', border: '#fcd34d', emoji: '🟠' },
  MEDIO:   { bg: '#FEF9C3', color: '#713f12', border: '#fde68a', emoji: '🟡' },
  BASSO:   { bg: '#DCFCE7', color: '#14532d', border: '#86efac', emoji: '🟢' },
};

// ── Colore notti buchi ────────────────────────────────────────────────────────
function colorNotti(n: number): { bg: string; color: string } {
  if (n <= 2) return { bg: '#FEE2E2', color: '#7f1d1d' };
  if (n <= 4) return { bg: '#FEF3C7', color: '#78350f' };
  return { bg: '#FEF9C3', color: '#713f12' };
}

// ── Formatta data ─────────────────────────────────────────────────────────────
function fmtDate(iso: string): string {
  return new Date(iso).toLocaleDateString('it-IT', {
    weekday: 'short', day: '2-digit', month: '2-digit', year: 'numeric',
  });
}

// ── Login ─────────────────────────────────────────────────────────────────────
function LoginForm({ onLogin }: { onLogin: () => void }) {
  const [pwd, setPwd]   = useState('');
  const [err, setErr]   = useState('');
  const [busy, setBusy] = useState(false);

  async function login() {
    setBusy(true); setErr('');
    const res = await fetch('/api/admin/login', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ password: pwd }),
    });
    if (res.ok) onLogin();
    else setErr('Password errata');
    setBusy(false);
  }

  return (
    <div style={{ maxWidth: 360, margin: '80px auto', padding: '0 20px' }}>
      <div style={{ ...card, padding: '28px 24px' }}>
        <p style={{ margin: '0 0 6px', fontSize: 20, fontWeight: 700, color: '#111' }}>🔒 Admin</p>
        <p style={{ margin: '0 0 20px', fontSize: 14, color: '#6b7280' }}>Pannello operativo</p>
        <input
          type="password" placeholder="Password"
          value={pwd} onChange={e => setPwd(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && login()}
          style={{
            width: '100%', padding: '10px 12px', fontSize: 15,
            border: '1px solid #d1d5db', borderRadius: 8,
            marginBottom: 10, boxSizing: 'border-box',
          }}
        />
        {err && <p style={{ fontSize: 13, color: '#dc2626', marginBottom: 8 }}>{err}</p>}
        <button style={{ ...btnG, width: '100%', opacity: busy ? 0.6 : 1 }}
          onClick={login} disabled={busy}>
          {busy ? 'Accesso…' : 'Accedi'}
        </button>
      </div>
    </div>
  );
}

// ── Tab Buchi ─────────────────────────────────────────────────────────────────
function TabBuchi({ buchi }: { buchi: Buco[] }) {
  if (buchi.length === 0) {
    return (
      <div style={{ ...card, textAlign: 'center', padding: '40px 20px' }}>
        <p style={{ margin: 0, fontSize: 28 }}>✅</p>
        <p style={{ margin: '8px 0 0', fontSize: 15, color: '#16a34a', fontWeight: 600 }}>
          Nessun buco trovato
        </p>
        <p style={{ margin: '4px 0 0', fontSize: 13, color: '#9ca3af' }}>
          Non ci sono finestre libere inferiori a 7 notti nei prossimi 12 mesi.
        </p>
      </div>
    );
  }

  // Raggruppa per property per leggibilità
  const properties = [...new Set(buchi.map(b => b.property))].sort();

  return (
    <div>
      <p style={{ margin: '0 0 14px', fontSize: 13, color: '#6b7280' }}>
        {buchi.length} finestre libere &lt; 7 notti nei prossimi 12 mesi
      </p>

      {/* Tabella */}
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: '#f9fafb' }}>
              {['Property', 'Stanza', 'Libera dal', 'Libera al', 'Notti'].map(h => (
                <th key={h} style={{
                  padding: '10px 12px', textAlign: 'left', fontWeight: 600,
                  color: '#6b7280', fontSize: 11, textTransform: 'uppercase',
                  letterSpacing: '0.04em', borderBottom: '1px solid #e5e7eb',
                }}>
                  {h}
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {buchi.map((b, i) => {
              const { bg, color } = colorNotti(b.notti);
              return (
                <tr key={i} style={{ borderBottom: '0.5px solid #f3f4f6' }}>
                  <td style={{ padding: '10px 12px', color: '#4b5563', fontSize: 12 }}>{b.property}</td>
                  <td style={{ padding: '10px 12px', fontWeight: 600, color: '#111' }}>{b.roomName}</td>
                  <td style={{ padding: '10px 12px', color: '#4b5563' }}>{fmtDate(b.liberaDal)}</td>
                  <td style={{ padding: '10px 12px', color: '#4b5563' }}>{fmtDate(b.liberaAl)}</td>
                  <td style={{ padding: '10px 12px' }}>
                    <span style={{
                      background: bg, color, fontWeight: 700,
                      padding: '3px 10px', borderRadius: 20, fontSize: 12,
                    }}>
                      {b.notti} {b.notti === 1 ? 'notte' : 'notti'}
                    </span>
                  </td>
                </tr>
              );
            })}
          </tbody>
        </table>
      </div>
    </div>
  );
}

// ── Tab Pulizie ───────────────────────────────────────────────────────────────
function TabPulizie({ pulizie }: { pulizie: PuliziaDay[] }) {
  const [filtroMese, setFiltroMese] = useState('Tutti');
  const [soloRischio, setSoloRischio] = useState(false);

  // Mesi disponibili
  const mesi = ['Tutti', ...Array.from(new Set(
    pulizie.map(p => {
      const d = new Date(p.date);
      return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
    })
  )).sort()];

  const mesiFmt: Record<string, string> = {};
  for (const m of mesi) {
    if (m === 'Tutti') { mesiFmt[m] = 'Tutti'; continue; }
    const [y, mo] = m.split('-');
    const d = new Date(Number(y), Number(mo) - 1, 1);
    mesiFmt[m] = d.toLocaleDateString('it-IT', { month: 'long', year: 'numeric' });
  }

  const filtered = pulizie.filter(p => {
    if (filtroMese !== 'Tutti') {
      const pm = p.date.slice(0, 7);
      if (pm !== filtroMese) return false;
    }
    if (soloRischio && !['CRITICO', 'ALTO'].includes(p.rischio)) return false;
    return true;
  });

  if (pulizie.length === 0) {
    return (
      <div style={{ ...card, textAlign: 'center', padding: '40px 20px' }}>
        <p style={{ margin: 0, fontSize: 13, color: '#9ca3af' }}>Nessuna partenza nei prossimi 12 mesi.</p>
      </div>
    );
  }

  return (
    <div>
      {/* Filtri */}
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        <select
          value={filtroMese}
          onChange={e => setFiltroMese(e.target.value)}
          style={{
            padding: '7px 12px', fontSize: 13, border: '0.5px solid #d1d5db',
            borderRadius: 8, background: '#fff', cursor: 'pointer',
          }}
        >
          {mesi.map(m => <option key={m} value={m}>{mesiFmt[m]}</option>)}
        </select>

        <label style={{ display: 'flex', gap: 6, alignItems: 'center', cursor: 'pointer', fontSize: 13, color: '#374151' }}>
          <input
            type="checkbox"
            checked={soloRischio}
            onChange={e => setSoloRischio(e.target.checked)}
            style={{ width: 15, height: 15 }}
          />
          Solo Critici e Alti
        </label>

        <span style={{ fontSize: 12, color: '#9ca3af', marginLeft: 'auto' }}>
          {filtered.length} giorn{filtered.length === 1 ? 'o' : 'i'}
        </span>
      </div>

      {/* Cards giornaliere */}
      {filtered.length === 0 && (
        <div style={{ ...card, textAlign: 'center', padding: '30px' }}>
          <p style={{ margin: 0, fontSize: 13, color: '#9ca3af' }}>Nessun giorno per il filtro selezionato.</p>
        </div>
      )}

      {filtered.map((day, i) => {
        const cfg = RISCHIO_CONFIG[day.rischio];
        return (
          <div key={i} style={{
            ...card,
            border: `1px solid ${cfg.border}`,
            background: cfg.bg,
          }}>
            {/* Header card */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#111' }}>
                {fmtDate(day.date)}
              </p>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <span style={{
                  background: cfg.color, color: '#fff', fontSize: 11, fontWeight: 700,
                  padding: '3px 10px', borderRadius: 20,
                }}>
                  {cfg.emoji} {day.rischio}
                </span>
              </div>
            </div>

            {/* Metriche */}
            <div style={{ display: 'flex', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
              {[
                { label: 'Partenze', value: day.departures.length, emoji: '✈️' },
                { label: 'Arrivi',   value: day.arrivals.length,   emoji: '🏠' },
                { label: 'MQ da pulire', value: `${day.mqTotale} mq`, emoji: '📐' },
              ].map(m => (
                <div key={m.label} style={{
                  background: 'rgba(255,255,255,0.7)', borderRadius: 8,
                  padding: '8px 14px', minWidth: 90,
                }}>
                  <p style={{ margin: 0, fontSize: 18, fontWeight: 700, color: cfg.color }}>
                    {m.emoji} {m.value}
                  </p>
                  <p style={{ margin: 0, fontSize: 11, color: '#6b7280' }}>{m.label}</p>
                </div>
              ))}
            </div>

            {/* Stanze che partono */}
            {day.departures.length > 0 && (
              <div style={{ marginBottom: 6 }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                  Partenze
                </span>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 }}>
                  {day.departures.map((r, j) => (
                    <span key={j} style={{
                      background: 'rgba(255,255,255,0.8)', border: `0.5px solid ${cfg.border}`,
                      borderRadius: 6, padding: '3px 10px', fontSize: 12, color: '#111',
                    }}>
                      {r.roomName} <span style={{ color: '#9ca3af' }}>{r.mq} mq</span>
                    </span>
                  ))}
                </div>
              </div>
            )}

            {/* Stanze che arrivano */}
            {day.arrivals.length > 0 && (
              <div>
                <span style={{ fontSize: 11, fontWeight: 600, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                  Arrivi stesso giorno
                </span>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 }}>
                  {day.arrivals.map((r, j) => (
                    <span key={j} style={{
                      background: 'rgba(255,255,255,0.5)', borderRadius: 6,
                      padding: '3px 10px', fontSize: 12, color: '#4b5563',
                      border: '0.5px solid #d1d5db',
                    }}>
                      {r.roomName}
                    </span>
                  ))}
                </div>
              </div>
            )}
          </div>
        );
      })}
    </div>
  );
}

// ── Componente principale ─────────────────────────────────────────────────────
export default function AdminOperativo() {
  const [authed, setAuthed]   = useState<boolean | null>(null);
  const [data, setData]       = useState<OperativoData | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError]     = useState('');
  const [activeTab, setActiveTab] = useState<'buchi' | 'pulizie'>('buchi');

  useEffect(() => {
    fetch('/api/admin/operativo')
      .then(r => {
        setAuthed(r.ok);
        if (r.ok) return r.json();
      })
      .then(d => { if (d) setData(d); })
      .catch(() => setAuthed(false));
  }, []);

  async function refresh() {
    setLoading(true); setError('');
    try {
      const res = await fetch('/api/admin/operativo');
      if (!res.ok) throw new Error(`Errore ${res.status}`);
      setData(await res.json());
    } catch (e: any) {
      setError(e.message);
    }
    setLoading(false);
  }

  async function logout() {
    await fetch('/api/admin/login', { method: 'DELETE' });
    setAuthed(false);
  }

  if (authed === null) {
    return <div style={{ padding: 40, textAlign: 'center', color: '#9ca3af' }}>Caricamento…</div>;
  }
  if (!authed) {
    return <LoginForm onLogin={() => { setAuthed(true); refresh(); }} />;
  }

  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '20px 16px 80px' }}>

      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: '#111' }}>Operativo</h1>
          {data && (
            <p style={{ margin: '2px 0 0', fontSize: 12, color: '#9ca3af' }}>
              {data.totalePrenotazioni} prenotazioni · aggiornato {new Date(data.lastUpdated).toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' })}
            </p>
          )}
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <a href="/admin" style={{ ...btnSec, textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>
            ← Admin
          </a>
          <button style={btnSec} onClick={refresh} disabled={loading}>
            {loading ? '…' : '↻ Aggiorna'}
          </button>
          <button style={btnSec} onClick={logout}>Esci</button>
        </div>
      </div>

      {error && (
        <div style={{ background: '#FEE2E2', border: '0.5px solid #fca5a5', borderRadius: 8, padding: '10px 14px', margin: '12px 0', fontSize: 13, color: '#7f1d1d' }}>
          ❌ {error}
        </div>
      )}

      {!data && !loading && !error && (
        <div style={{ ...card, textAlign: 'center', padding: '40px', marginTop: 20 }}>
          <p style={{ margin: 0, fontSize: 13, color: '#9ca3af' }}>Nessun dato. Clicca Aggiorna.</p>
        </div>
      )}

      {loading && (
        <div style={{ padding: '40px', textAlign: 'center', color: '#9ca3af', fontSize: 13 }}>
          Caricamento prenotazioni da Beds24…
        </div>
      )}

      {data && (
        <>
          {/* Counter badge */}
          <div style={{ display: 'flex', gap: 10, margin: '16px 0', flexWrap: 'wrap' }}>
            {[
              { label: 'Buchi < 7 notti', value: data.buchi.length, color: data.buchi.length > 0 ? '#7f1d1d' : '#14532d', bg: data.buchi.length > 0 ? '#FEE2E2' : '#DCFCE7' },
              { label: 'Giorni con pulizie', value: data.pulizie.length, color: '#1e40af', bg: '#DBEAFE' },
              { label: 'Giorni CRITICO/ALTO', value: data.pulizie.filter(p => ['CRITICO','ALTO'].includes(p.rischio)).length, color: '#78350f', bg: '#FEF3C7' },
            ].map(s => (
              <div key={s.label} style={{ background: s.bg, borderRadius: 8, padding: '8px 16px' }}>
                <p style={{ margin: 0, fontSize: 20, fontWeight: 700, color: s.color }}>{s.value}</p>
                <p style={{ margin: 0, fontSize: 11, color: '#6b7280' }}>{s.label}</p>
              </div>
            ))}
          </div>

          {/* Tabs */}
          <div style={{ display: 'flex', borderBottom: '1px solid #e5e7eb', marginBottom: 20 }}>
            {([
              { key: 'buchi',   label: `Buchi (${data.buchi.length})` },
              { key: 'pulizie', label: `Pulizie (${data.pulizie.length})` },
            ] as const).map(tab => (
              <button
                key={tab.key}
                onClick={() => setActiveTab(tab.key)}
                style={{
                  padding: '10px 20px', fontSize: 14, fontWeight: 600,
                  background: 'none', border: 'none', cursor: 'pointer',
                  borderBottom: activeTab === tab.key ? '2px solid #1E73BE' : '2px solid transparent',
                  color: activeTab === tab.key ? '#1E73BE' : '#6b7280',
                }}
              >
                {tab.label}
              </button>
            ))}
          </div>

          {activeTab === 'buchi'   && <TabBuchi   buchi={data.buchi} />}
          {activeTab === 'pulizie' && <TabPulizie pulizie={data.pulizie} />}
        </>
      )}
    </div>
  );
}
